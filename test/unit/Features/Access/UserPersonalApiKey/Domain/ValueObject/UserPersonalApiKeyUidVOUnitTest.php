...
final class UserPersonalApiKeyUidVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = UserPersonalApiKeyUidVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = UserPersonalApiKeyUidVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = UserPersonalApiKeyUidVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = UserPersonalApiKeyUidVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(UserPersonalApiKeyUidVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = UserPersonalApiKeyUidVO::from('one');
        $same = UserPersonalApiKeyUidVO::from('one');
        $other = UserPersonalApiKeyUidVO::from('other');
