...
final class UserPersonalApiKeyKeyHashVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = UserPersonalApiKeyKeyHashVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = UserPersonalApiKeyKeyHashVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = UserPersonalApiKeyKeyHashVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = UserPersonalApiKeyKeyHashVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(UserPersonalApiKeyKeyHashVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = UserPersonalApiKeyKeyHashVO::from('one');
        $same = UserPersonalApiKeyKeyHashVO::from('one');
        $other = UserPersonalApiKeyKeyHashVO::from('other');
