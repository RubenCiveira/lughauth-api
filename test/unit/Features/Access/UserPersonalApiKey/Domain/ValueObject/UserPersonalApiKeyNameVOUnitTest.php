...
final class UserPersonalApiKeyNameVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = UserPersonalApiKeyNameVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = UserPersonalApiKeyNameVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = UserPersonalApiKeyNameVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = UserPersonalApiKeyNameVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(UserPersonalApiKeyNameVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = UserPersonalApiKeyNameVO::from('one');
        $same = UserPersonalApiKeyNameVO::from('one');
        $other = UserPersonalApiKeyNameVO::from('other');
