...
final class UserPersonalApiKeyScopesVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = UserPersonalApiKeyScopesVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = UserPersonalApiKeyScopesVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = UserPersonalApiKeyScopesVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = UserPersonalApiKeyScopesVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(UserPersonalApiKeyScopesVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = UserPersonalApiKeyScopesVO::from('one');
        $same = UserPersonalApiKeyScopesVO::from('one');
        $other = UserPersonalApiKeyScopesVO::from('other');
