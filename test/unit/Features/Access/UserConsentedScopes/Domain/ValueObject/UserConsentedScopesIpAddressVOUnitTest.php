...
final class UserConsentedScopesIpAddressVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = UserConsentedScopesIpAddressVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = UserConsentedScopesIpAddressVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = UserConsentedScopesIpAddressVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = UserConsentedScopesIpAddressVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(UserConsentedScopesIpAddressVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = UserConsentedScopesIpAddressVO::from('one');
        $same = UserConsentedScopesIpAddressVO::from('one');
        $other = UserConsentedScopesIpAddressVO::from('other');
