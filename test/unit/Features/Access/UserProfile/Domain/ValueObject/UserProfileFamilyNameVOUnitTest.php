...
final class UserProfileFamilyNameVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = UserProfileFamilyNameVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = UserProfileFamilyNameVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = UserProfileFamilyNameVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = UserProfileFamilyNameVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(UserProfileFamilyNameVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = UserProfileFamilyNameVO::from('one');
        $same = UserProfileFamilyNameVO::from('one');
        $other = UserProfileFamilyNameVO::from('other');
