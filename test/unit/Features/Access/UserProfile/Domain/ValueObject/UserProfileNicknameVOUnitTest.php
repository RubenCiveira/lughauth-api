...
final class UserProfileNicknameVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = UserProfileNicknameVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = UserProfileNicknameVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = UserProfileNicknameVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = UserProfileNicknameVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(UserProfileNicknameVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = UserProfileNicknameVO::from('one');
        $same = UserProfileNicknameVO::from('one');
        $other = UserProfileNicknameVO::from('other');
