...
final class UserProfileLocaleVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = UserProfileLocaleVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = UserProfileLocaleVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = UserProfileLocaleVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = UserProfileLocaleVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(UserProfileLocaleVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = UserProfileLocaleVO::from('one');
        $same = UserProfileLocaleVO::from('one');
        $other = UserProfileLocaleVO::from('other');
