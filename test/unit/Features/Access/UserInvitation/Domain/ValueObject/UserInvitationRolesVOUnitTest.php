...
final class UserInvitationRolesVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = UserInvitationRolesVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = UserInvitationRolesVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = UserInvitationRolesVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = UserInvitationRolesVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(UserInvitationRolesVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = UserInvitationRolesVO::from('one');
        $same = UserInvitationRolesVO::from('one');
        $other = UserInvitationRolesVO::from('other');
