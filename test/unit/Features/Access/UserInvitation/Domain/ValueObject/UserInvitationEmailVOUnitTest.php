...
final class UserInvitationEmailVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = UserInvitationEmailVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = UserInvitationEmailVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = UserInvitationEmailVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = UserInvitationEmailVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(UserInvitationEmailVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = UserInvitationEmailVO::from('one');
        $same = UserInvitationEmailVO::from('one');
        $other = UserInvitationEmailVO::from('other');
