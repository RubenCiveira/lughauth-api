...
final class UserMfaRecoveryCodeUidVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = UserMfaRecoveryCodeUidVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = UserMfaRecoveryCodeUidVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = UserMfaRecoveryCodeUidVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = UserMfaRecoveryCodeUidVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(UserMfaRecoveryCodeUidVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = UserMfaRecoveryCodeUidVO::from('one');
        $same = UserMfaRecoveryCodeUidVO::from('one');
        $other = UserMfaRecoveryCodeUidVO::from('other');
