...
final class UserAcceptedTermnsOfUseUserAgentVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = UserAcceptedTermnsOfUseUserAgentVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = UserAcceptedTermnsOfUseUserAgentVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = UserAcceptedTermnsOfUseUserAgentVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = UserAcceptedTermnsOfUseUserAgentVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(UserAcceptedTermnsOfUseUserAgentVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = UserAcceptedTermnsOfUseUserAgentVO::from('one');
        $same = UserAcceptedTermnsOfUseUserAgentVO::from('one');
        $other = UserAcceptedTermnsOfUseUserAgentVO::from('other');
