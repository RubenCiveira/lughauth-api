...
final class TrustedClientRequirePkceVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = true;
        $ref = TrustedClientRequirePkceVO::from($value);
        $this->assertEquals(true, $ref->value());
        $other = TrustedClientRequirePkceVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = TrustedClientRequirePkceVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = TrustedClientRequirePkceVO::tryFrom('1', $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(TrustedClientRequirePkceVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = TrustedClientRequirePkceVO::from(true);
        $same = TrustedClientRequirePkceVO::from(true);
        $other = TrustedClientRequirePkceVO::from(false);
