...
final class TrustedClientIsResourceServerVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = true;
        $ref = TrustedClientIsResourceServerVO::from($value);
        $this->assertEquals(true, $ref->value());
        $other = TrustedClientIsResourceServerVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = TrustedClientIsResourceServerVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = TrustedClientIsResourceServerVO::tryFrom('1', $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(TrustedClientIsResourceServerVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = TrustedClientIsResourceServerVO::from(true);
        $same = TrustedClientIsResourceServerVO::from(true);
        $other = TrustedClientIsResourceServerVO::from(false);
