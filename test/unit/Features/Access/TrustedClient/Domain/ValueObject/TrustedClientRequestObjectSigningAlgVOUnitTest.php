...
final class TrustedClientRequestObjectSigningAlgVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = TrustedClientRequestObjectSigningAlgVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = TrustedClientRequestObjectSigningAlgVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = TrustedClientRequestObjectSigningAlgVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = TrustedClientRequestObjectSigningAlgVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(TrustedClientRequestObjectSigningAlgVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = TrustedClientRequestObjectSigningAlgVO::from('one');
        $same = TrustedClientRequestObjectSigningAlgVO::from('one');
        $other = TrustedClientRequestObjectSigningAlgVO::from('other');
