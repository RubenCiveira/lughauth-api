...
final class TrustedClientClientUriVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = TrustedClientClientUriVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = TrustedClientClientUriVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = TrustedClientClientUriVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = TrustedClientClientUriVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(TrustedClientClientUriVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = TrustedClientClientUriVO::from('one');
        $same = TrustedClientClientUriVO::from('one');
        $other = TrustedClientClientUriVO::from('other');
