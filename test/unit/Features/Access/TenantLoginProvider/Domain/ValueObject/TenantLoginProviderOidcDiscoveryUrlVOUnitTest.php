...
final class TenantLoginProviderOidcDiscoveryUrlVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = TenantLoginProviderOidcDiscoveryUrlVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = TenantLoginProviderOidcDiscoveryUrlVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = TenantLoginProviderOidcDiscoveryUrlVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = TenantLoginProviderOidcDiscoveryUrlVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(TenantLoginProviderOidcDiscoveryUrlVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = TenantLoginProviderOidcDiscoveryUrlVO::from('one');
        $same = TenantLoginProviderOidcDiscoveryUrlVO::from('one');
        $other = TenantLoginProviderOidcDiscoveryUrlVO::from('other');
