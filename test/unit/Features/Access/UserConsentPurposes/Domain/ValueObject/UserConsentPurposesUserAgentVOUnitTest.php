...
final class UserConsentPurposesUserAgentVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = UserConsentPurposesUserAgentVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = UserConsentPurposesUserAgentVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = UserConsentPurposesUserAgentVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = UserConsentPurposesUserAgentVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(UserConsentPurposesUserAgentVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = UserConsentPurposesUserAgentVO::from('one');
        $same = UserConsentPurposesUserAgentVO::from('one');
        $other = UserConsentPurposesUserAgentVO::from('other');
