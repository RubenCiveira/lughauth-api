...
final class UserConsentPurposesIpAddressVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = UserConsentPurposesIpAddressVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = UserConsentPurposesIpAddressVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = UserConsentPurposesIpAddressVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = UserConsentPurposesIpAddressVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(UserConsentPurposesIpAddressVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = UserConsentPurposesIpAddressVO::from('one');
        $same = UserConsentPurposesIpAddressVO::from('one');
        $other = UserConsentPurposesIpAddressVO::from('other');
