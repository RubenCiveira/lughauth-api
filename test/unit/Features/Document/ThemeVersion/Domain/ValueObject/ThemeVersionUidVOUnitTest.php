...
final class ThemeVersionUidVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = ThemeVersionUidVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = ThemeVersionUidVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = ThemeVersionUidVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = ThemeVersionUidVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(ThemeVersionUidVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = ThemeVersionUidVO::from('one');
        $same = ThemeVersionUidVO::from('one');
        $other = ThemeVersionUidVO::from('other');
