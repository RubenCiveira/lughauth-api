...
final class SnippetVersionLocaleVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = SnippetVersionLocaleVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = SnippetVersionLocaleVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = SnippetVersionLocaleVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = SnippetVersionLocaleVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(SnippetVersionLocaleVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = SnippetVersionLocaleVO::from('one');
        $same = SnippetVersionLocaleVO::from('one');
        $other = SnippetVersionLocaleVO::from('other');
