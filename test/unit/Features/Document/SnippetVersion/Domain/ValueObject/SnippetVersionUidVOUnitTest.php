...
final class SnippetVersionUidVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = SnippetVersionUidVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = SnippetVersionUidVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = SnippetVersionUidVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = SnippetVersionUidVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(SnippetVersionUidVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = SnippetVersionUidVO::from('one');
        $same = SnippetVersionUidVO::from('one');
        $other = SnippetVersionUidVO::from('other');
