...
final class ThemeAssetCodeVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = ThemeAssetCodeVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = ThemeAssetCodeVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = ThemeAssetCodeVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = ThemeAssetCodeVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(ThemeAssetCodeVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = ThemeAssetCodeVO::from('one');
        $same = ThemeAssetCodeVO::from('one');
        $other = ThemeAssetCodeVO::from('other');
