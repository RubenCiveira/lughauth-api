...
final class ThemeAssetUidVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = ThemeAssetUidVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = ThemeAssetUidVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = ThemeAssetUidVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = ThemeAssetUidVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(ThemeAssetUidVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = ThemeAssetUidVO::from('one');
        $same = ThemeAssetUidVO::from('one');
        $other = ThemeAssetUidVO::from('other');
