...
final class ThemeAssetTypeVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = ThemeAssetTypeVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = ThemeAssetTypeVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = ThemeAssetTypeVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = ThemeAssetTypeVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(ThemeAssetTypeVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = ThemeAssetTypeVO::from('one');
        $same = ThemeAssetTypeVO::from('one');
        $other = ThemeAssetTypeVO::from('other');
