...
namespace Civi\Lughauth\Bootstrap\Middleware\Metrics {
    final class MetricsRotatorTestHook
    {
        public static bool $forceGlobFalse = false;
        public static int $globCallsUntilFail = 0;
    }

    if (!function_exists(__NAMESPACE__ . '\\glob')) {
        function glob(string $pattern, int $flags = 0): array|false
        {
            if (MetricsRotatorTestHook::$forceGlobFalse) {
                if (MetricsRotatorTestHook::$globCallsUntilFail <= 0) {
                    return false;
                }
                MetricsRotatorTestHook::$globCallsUntilFail--;
            }
...
namespace {
    ...
    use Civi\Lughauth\Bootstrap\Middleware\Metrics\MetricsRotatorTestHook;
    ...
    final class MetricsRotatorUnitTest extends TestCase
    {
        ...
        /**
         * Ensures rotateAll handles a glob failure on the root directory.
         */
        public function testRotateAllHandlesGlobFalse(): void
        {
            /*
             * Arrange: create a root and force glob to return false.
             */
            $root = sys_get_temp_dir() . '/rotator_' . uniqid();
            mkdir($root, 0777, true);
            $rotator = new MetricsRotator($root, ['raw' => 1], false);

            MetricsRotatorTestHook::$forceGlobFalse = true;
            MetricsRotatorTestHook::$globCallsUntilFail = 0;
            try {
                /*
                 * Act: rotate all metrics while glob fails.
                 */
                $rotator->rotateAll();
            } finally {
                MetricsRotatorTestHook::$forceGlobFalse = false;
            }
        ...
        /**
         * Ensures processMetric handles a glob failure on the series directory.
         */
        public function testProcessMetricHandlesGlobFalse(): void
        {
            /*
             * Arrange: create a metric directory and force glob to fail for series.
             */
            $root = sys_get_temp_dir() . '/rotator_' . uniqid();
            $metricDir = $root . '/metric';
            mkdir($metricDir . '/series/aa/bb', 0777, true);

            $rotator = new MetricsRotator($root, ['raw' => 1], false);
            $method = new ReflectionMethod($rotator, 'processMetric');

            MetricsRotatorTestHook::$forceGlobFalse = true;
            MetricsRotatorTestHook::$globCallsUntilFail = 0;
            try {
                /*
                 * Act: invoke processMetric while the series glob fails.
                 */
                $method->invoke($rotator, $metricDir);
            } finally {
                MetricsRotatorTestHook::$forceGlobFalse = false;
            }

            /*
             * Assert: verify the series directory is untouched.
             */
            $this->assertTrue(is_dir($metricDir));
        }

        /**
         * Ensures processSeries handles a glob failure on year directories.
         */
        public function testProcessSeriesHandlesYearGlobFalse(): void
        {
            /*
             * Arrange: create a series dir with a raw partition.
             */
            $root = sys_get_temp_dir() . '/rotator_' . uniqid();
            $seriesDir = $root . '/series';
            mkdir($seriesDir . '/raw', 0777, true);

            $rotator = new MetricsRotator($root, ['raw' => 1], false);
            $method = new ReflectionMethod($rotator, 'processSeries');

            MetricsRotatorTestHook::$forceGlobFalse = true;
            MetricsRotatorTestHook::$globCallsUntilFail = 0;
            try {
                /*
                 * Act: invoke processSeries while the year glob fails.
                 */
                $method->invoke($rotator, $seriesDir);
            } finally {
                MetricsRotatorTestHook::$forceGlobFalse = false;
            }

            /*
             * Assert: verify no files were modified.
             */
            $this->assertTrue(is_dir($seriesDir));
        }

        /**
         * Ensures processSeries handles a glob failure on month directories.
         */
        public function testProcessSeriesHandlesMonthGlobFalse(): void
        {
            /*
             * Arrange: create a series dir with a raw/2020 partition.
             */
            $root = sys_get_temp_dir() . '/rotator_' . uniqid();
            $seriesDir = $root . '/series';
            mkdir($seriesDir . '/raw/2020', 0777, true);

            $rotator = new MetricsRotator($root, ['raw' => 1], false);
            $method = new ReflectionMethod($rotator, 'processSeries');

            MetricsRotatorTestHook::$forceGlobFalse = true;
            MetricsRotatorTestHook::$globCallsUntilFail = 1;
            try {
                /*
                 * Act: first glob (year dirs) succeeds, second (month dirs) fails.
                 */
                $method->invoke($rotator, $seriesDir);
            } finally {
                MetricsRotatorTestHook::$forceGlobFalse = false;
             ...
             * Assert: verify no files were modified.
             */
            $this->assertTrue(is_dir($seriesDir . '/raw/2020'));
        }

        /**
         * Ensures processSeries handles a glob failure on day files.
         */
        public function testProcessSeriesHandlesDayFileGlobFalse(): void
        {
            /*
             * Arrange: create a series dir with a raw/2020/01 partition.
             */
            $root = sys_get_temp_dir() . '/rotator_' . uniqid();
            $seriesDir = $root . '/series';
            mkdir($seriesDir . '/raw/2020/01', 0777, true);

            $rotator = new MetricsRotator($root, ['raw' => 1], false);
            $method = new ReflectionMethod($rotator, 'processSeries');

            MetricsRotatorTestHook::$forceGlobFalse = true;
            MetricsRotatorTestHook::$globCallsUntilFail = 2;
            try {
                /*
                 * Act: year and month globs succeed, day files glob fails.
                 */
                $method->invoke($rotator, $seriesDir);
            } finally {
                MetricsRotatorTestHook::$forceGlobFalse = false;
             ...
             * Assert: verify no files were modified.
             */
            $this->assertTrue(is_dir($seriesDir . '/raw/2020/01'));
        }
