...
final class MessageUidVOUnitTest extends TestCase
{
  public function test_asignation_keep_value(): void
  {
    $value = 'one';
    $ref = MessageUidVO::from( $value );
    $this->assertEquals('one', $ref->value());
    $other = MessageUidVO::tryFrom($ref, new ConstraintFailList() );
    $this->assertSame($other, $ref);
    $more = MessageUidVO::from($ref);
    $this->assertSame($more, $ref);
  }
  public function test_asignation_invalid_type(): void
  {
    $errors = new ConstraintFailList();
    $other = MessageUidVO::tryFrom(1, $errors);
  ...
  public function test_optimist_asignation_invalid_type(): void
  {
    ...
    $method = new ReflectionMethod(MessageUidVO::class, 'fromUnsafe');
    $method->invoke(null, [11, "bad"]);
  }
  public function test_equals(): void

  {
     $one = MessageUidVO::from('one');     $same = MessageUidVO::from('one');     $other = MessageUidVO::from('other');
