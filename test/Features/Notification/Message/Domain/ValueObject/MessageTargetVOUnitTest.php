...
final class MessageTargetVOUnitTest extends TestCase
{
  public function test_asignation_keep_value(): void
  {
    $value = 'one';
    $ref = MessageTargetVO::from( $value );
    $this->assertEquals('one', $ref->value());
    $other = MessageTargetVO::tryFrom($ref, new ConstraintFailList() );
    $this->assertSame($other, $ref);
    $more = MessageTargetVO::from($ref);
    $this->assertSame($more, $ref);
  }
  public function test_asignation_invalid_type(): void
  {
    $errors = new ConstraintFailList();
    $other = MessageTargetVO::tryFrom(1, $errors);
  ...
  public function test_optimist_asignation_invalid_type(): void
  {
    ...
    $method = new ReflectionMethod(MessageTargetVO::class, 'fromUnsafe');
    $method->invoke(null, [11, "bad"]);
  }
  public function test_equals(): void

  {
     $one = MessageTargetVO::from('one');     $same = MessageTargetVO::from('one');     $other = MessageTargetVO::from('other');
