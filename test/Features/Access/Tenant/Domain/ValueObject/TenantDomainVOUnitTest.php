...
final class TenantDomainVOUnitTest extends TestCase
{
  public function test_asignation_keep_value(): void
  {
    $value = 'one';
    $ref = TenantDomainVO::from( $value );
    $this->assertEquals('one', $ref->value());
    $other = TenantDomainVO::tryFrom($ref, new ConstraintFailList() );
    $this->assertSame($other, $ref);
    $more = TenantDomainVO::from($ref);
    $this->assertSame($more, $ref);
  }
  public function test_asignation_invalid_type(): void
  {
    $errors = new ConstraintFailList();
    $other = TenantDomainVO::tryFrom(1, $errors);
  ...
  public function test_optimist_asignation_invalid_type(): void
  {
    ...
    $method = new ReflectionMethod(TenantDomainVO::class, 'fromUnsafe');
    $method->invoke(null, [11, "bad"]);
  }
  public function test_equals(): void

  {
     $one = TenantDomainVO::from('one');     $same = TenantDomainVO::from('one');     $other = TenantDomainVO::from('other');
