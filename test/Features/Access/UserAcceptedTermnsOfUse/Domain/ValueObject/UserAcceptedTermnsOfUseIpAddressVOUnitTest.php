...
final class UserAcceptedTermnsOfUseIpAddressVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = UserAcceptedTermnsOfUseIpAddressVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = UserAcceptedTermnsOfUseIpAddressVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = UserAcceptedTermnsOfUseIpAddressVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = UserAcceptedTermnsOfUseIpAddressVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(UserAcceptedTermnsOfUseIpAddressVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = UserAcceptedTermnsOfUseIpAddressVO::from('one');
        $same = UserAcceptedTermnsOfUseIpAddressVO::from('one');
        $other = UserAcceptedTermnsOfUseIpAddressVO::from('other');
