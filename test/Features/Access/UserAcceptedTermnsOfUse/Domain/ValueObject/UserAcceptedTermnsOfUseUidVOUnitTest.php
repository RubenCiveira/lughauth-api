...
final class UserAcceptedTermnsOfUseUidVOUnitTest extends TestCase
{
  public function test_asignation_keep_value(): void
  {
    $value = 'one';
    $ref = UserAcceptedTermnsOfUseUidVO::from( $value );
    $this->assertEquals('one', $ref->value());
    $other = UserAcceptedTermnsOfUseUidVO::tryFrom($ref, new ConstraintFailList() );
    $this->assertSame($other, $ref);
    $more = UserAcceptedTermnsOfUseUidVO::from($ref);
    $this->assertSame($more, $ref);
  }
  public function test_asignation_invalid_type(): void
  {
    $errors = new ConstraintFailList();
    $other = UserAcceptedTermnsOfUseUidVO::tryFrom(1, $errors);
  ...
  public function test_optimist_asignation_invalid_type(): void
  {
    ...
    $method = new ReflectionMethod(UserAcceptedTermnsOfUseUidVO::class, 'fromUnsafe');
    $method->invoke(null, [11, "bad"]);
  }
  public function test_equals(): void

  {
     $one = UserAcceptedTermnsOfUseUidVO::from('one');     $same = UserAcceptedTermnsOfUseUidVO::from('one');     $other = UserAcceptedTermnsOfUseUidVO::from('other');
