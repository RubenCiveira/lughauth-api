...
final class SecurityScopeResourceVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = SecurityScopeResourceVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = SecurityScopeResourceVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = SecurityScopeResourceVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = SecurityScopeResourceVO::tryFrom(1, $errors);
