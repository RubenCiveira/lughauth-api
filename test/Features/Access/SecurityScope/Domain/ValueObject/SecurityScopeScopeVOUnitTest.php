...
final class SecurityScopeScopeVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = SecurityScopeScopeVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = SecurityScopeScopeVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = SecurityScopeScopeVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = SecurityScopeScopeVO::tryFrom(1, $errors);
