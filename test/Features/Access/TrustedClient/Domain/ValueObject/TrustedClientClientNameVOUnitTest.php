...
final class TrustedClientClientNameVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = TrustedClientClientNameVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = TrustedClientClientNameVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = TrustedClientClientNameVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = TrustedClientClientNameVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(TrustedClientClientNameVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = TrustedClientClientNameVO::from('one');
        $same = TrustedClientClientNameVO::from('one');
        $other = TrustedClientClientNameVO::from('other');
