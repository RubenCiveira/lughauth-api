...
final class UserWebauthnCredentialAaguidVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = UserWebauthnCredentialAaguidVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = UserWebauthnCredentialAaguidVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = UserWebauthnCredentialAaguidVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = UserWebauthnCredentialAaguidVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(UserWebauthnCredentialAaguidVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = UserWebauthnCredentialAaguidVO::from('one');
        $same = UserWebauthnCredentialAaguidVO::from('one');
        $other = UserWebauthnCredentialAaguidVO::from('other');
