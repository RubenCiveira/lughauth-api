...
use Civi\Lughauth\Features\Access\Master\CustomRole\Domain\ValueObject\CustomRoleUidVO;

final class CustomRoleUidVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = CustomRoleUidVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = CustomRoleUidVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = CustomRoleUidVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = CustomRoleUidVO::tryFrom(1, $errors);
