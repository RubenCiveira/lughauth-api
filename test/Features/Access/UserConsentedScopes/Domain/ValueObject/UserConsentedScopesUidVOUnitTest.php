...
final class UserConsentedScopesUidVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = UserConsentedScopesUidVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = UserConsentedScopesUidVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = UserConsentedScopesUidVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = UserConsentedScopesUidVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(UserConsentedScopesUidVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = UserConsentedScopesUidVO::from('one');
        $same = UserConsentedScopesUidVO::from('one');
        $other = UserConsentedScopesUidVO::from('other');
