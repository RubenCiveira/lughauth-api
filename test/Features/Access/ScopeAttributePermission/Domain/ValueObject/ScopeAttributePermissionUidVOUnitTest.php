...
final class ScopeAttributePermissionUidVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = ScopeAttributePermissionUidVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = ScopeAttributePermissionUidVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = ScopeAttributePermissionUidVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = ScopeAttributePermissionUidVO::tryFrom(1, $errors);
