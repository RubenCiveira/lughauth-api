...
final class UserProfileMiddleNameVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = UserProfileMiddleNameVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = UserProfileMiddleNameVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = UserProfileMiddleNameVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = UserProfileMiddleNameVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(UserProfileMiddleNameVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = UserProfileMiddleNameVO::from('one');
        $same = UserProfileMiddleNameVO::from('one');
        $other = UserProfileMiddleNameVO::from('other');
