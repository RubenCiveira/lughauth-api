...
final class SecurityAttributeUidVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = SecurityAttributeUidVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = SecurityAttributeUidVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = SecurityAttributeUidVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = SecurityAttributeUidVO::tryFrom(1, $errors);
