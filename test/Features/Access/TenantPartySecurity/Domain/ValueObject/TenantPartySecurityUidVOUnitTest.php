...
final class TenantPartySecurityUidVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = TenantPartySecurityUidVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = TenantPartySecurityUidVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = TenantPartySecurityUidVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = TenantPartySecurityUidVO::tryFrom(1, $errors);
