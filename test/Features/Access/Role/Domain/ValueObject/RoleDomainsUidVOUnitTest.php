...
final class RoleDomainsUidVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = RoleDomainsUidVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = RoleDomainsUidVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = RoleDomainsUidVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = RoleDomainsUidVO::tryFrom(1, $errors);
