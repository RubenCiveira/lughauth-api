...
final class ScopeAssignationUidVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = ScopeAssignationUidVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = ScopeAssignationUidVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = ScopeAssignationUidVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = ScopeAssignationUidVO::tryFrom(1, $errors);
