...
final class PlatformIdentityRolesUidVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = PlatformIdentityRolesUidVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = PlatformIdentityRolesUidVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = PlatformIdentityRolesUidVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = PlatformIdentityRolesUidVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(PlatformIdentityRolesUidVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = PlatformIdentityRolesUidVO::from('one');
        $same = PlatformIdentityRolesUidVO::from('one');
        $other = PlatformIdentityRolesUidVO::from('other');
