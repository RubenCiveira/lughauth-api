...
final class UserRoleAssignamentVersionVOUnitTest extends TestCase
{
  public function test_asignation_keep_value(): void
  {
    $value = 1;
    $ref = UserRoleAssignamentVersionVO::from( $value );
    $this->assertEquals(1, $ref->value());
    $other = UserRoleAssignamentVersionVO::tryFrom($ref, new ConstraintFailList() );
    $this->assertSame($other, $ref);
    $more = UserRoleAssignamentVersionVO::from($ref);
    $this->assertSame($more, $ref);
  }
  public function test_asignation_invalid_type(): void
  {
    $errors = new ConstraintFailList();
    $other = UserRoleAssignamentVersionVO::tryFrom('1', $errors);
  ...
  public function test_optimist_asignation_invalid_type(): void
  {
    ...
    $method = new ReflectionMethod(UserRoleAssignamentVersionVO::class, 'fromUnsafe');
    $method->invoke(null, [11, "bad"]);
  }
  public function test_equals(): void

  {
     $one = UserRoleAssignamentVersionVO::from(1);     $same = UserRoleAssignamentVersionVO::from(1);     $other = UserRoleAssignamentVersionVO::from(2);
  ...
  {
    $ref = UserRoleAssignamentVersionVO::empty();
    $this->assertNull($ref->value());
  }
  public function test_try_from_empty(): void
  {
    $ref = UserRoleAssignamentVersionVO::tryFrom(null, new ConstraintFailList() );
    $this->assertNull($ref->value());
  }
}
