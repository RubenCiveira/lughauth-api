...
final class UserRoleAssignamentUidVOUnitTest extends TestCase
{
  public function test_asignation_keep_value(): void
  {
    $value = 'one';
    $ref = UserRoleAssignamentUidVO::from( $value );
    $this->assertEquals('one', $ref->value());
    $other = UserRoleAssignamentUidVO::tryFrom($ref, new ConstraintFailList() );
    $this->assertSame($other, $ref);
    $more = UserRoleAssignamentUidVO::from($ref);
    $this->assertSame($more, $ref);
  }
  public function test_asignation_invalid_type(): void
  {
    $errors = new ConstraintFailList();
    $other = UserRoleAssignamentUidVO::tryFrom(1, $errors);
  ...
  public function test_optimist_asignation_invalid_type(): void
  {
    ...
    $method = new ReflectionMethod(UserRoleAssignamentUidVO::class, 'fromUnsafe');
    $method->invoke(null, [11, "bad"]);
  }
  public function test_equals(): void

  {
     $one = UserRoleAssignamentUidVO::from('one');     $same = UserRoleAssignamentUidVO::from('one');     $other = UserRoleAssignamentUidVO::from('other');
