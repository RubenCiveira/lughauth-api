...
final class SecurityDomainNameVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = SecurityDomainNameVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = SecurityDomainNameVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = SecurityDomainNameVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = SecurityDomainNameVO::tryFrom(1, $errors);
