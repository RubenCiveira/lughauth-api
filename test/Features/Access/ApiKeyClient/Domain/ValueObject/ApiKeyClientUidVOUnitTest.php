...
final class ApiKeyClientUidVOUnitTest extends TestCase
{
  public function test_asignation_keep_value(): void
  {
    $value = 'one';
    $ref = ApiKeyClientUidVO::from( $value );
    $this->assertEquals('one', $ref->value());
    $other = ApiKeyClientUidVO::tryFrom($ref, new ConstraintFailList() );
    $this->assertSame($other, $ref);
    $more = ApiKeyClientUidVO::from($ref);
    $this->assertSame($more, $ref);
  }
  public function test_asignation_invalid_type(): void
  {
    $errors = new ConstraintFailList();
    $other = ApiKeyClientUidVO::tryFrom(1, $errors);
  ...
  {
     $one = ApiKeyClientUidVO::from('one');     $same = ApiKeyClientUidVO::from('one');     $other = ApiKeyClientUidVO::from('other');
