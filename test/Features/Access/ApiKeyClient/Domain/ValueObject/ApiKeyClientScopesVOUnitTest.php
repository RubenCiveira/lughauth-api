...
final class ApiKeyClientScopesVOUnitTest extends TestCase
{
  public function test_asignation_keep_value(): void
  {
    $value = 'one';
    $ref = ApiKeyClientScopesVO::from( $value );
    $this->assertEquals('one', $ref->value());
    $other = ApiKeyClientScopesVO::tryFrom($ref, new ConstraintFailList() );
    $this->assertSame($other, $ref);
    $more = ApiKeyClientScopesVO::from($ref);
    $this->assertSame($more, $ref);
  }
  public function test_asignation_invalid_type(): void
  {
    $errors = new ConstraintFailList();
    $other = ApiKeyClientScopesVO::tryFrom(1, $errors);
  ...
  {
     $one = ApiKeyClientScopesVO::from('one');     $same = ApiKeyClientScopesVO::from('one');     $other = ApiKeyClientScopesVO::from('other');
