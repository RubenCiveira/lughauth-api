...
final class RelyingPartyApiKeyVOUnitTest extends TestCase
{
  public function test_asignation_keep_value(): void
  {
    $value = 'one';
    $ref = RelyingPartyApiKeyVO::from( $value );
    $this->assertEquals('one', $ref->value());
    $other = RelyingPartyApiKeyVO::tryFrom($ref, new ConstraintFailList() );
    $this->assertSame($other, $ref);
    $more = RelyingPartyApiKeyVO::from($ref);
    $this->assertSame($more, $ref);
  }
  public function test_asignation_invalid_type(): void
  {
    $errors = new ConstraintFailList();
    $other = RelyingPartyApiKeyVO::tryFrom(1, $errors);
  ...
  public function test_optimist_asignation_invalid_type(): void
  {
    ...
    $method = new ReflectionMethod(RelyingPartyApiKeyVO::class, 'fromUnsafe');
    $method->invoke(null, [11, "bad"]);
  }
  public function test_equals(): void

  {
     $one = RelyingPartyApiKeyVO::from('one');     $same = RelyingPartyApiKeyVO::from('one');     $other = RelyingPartyApiKeyVO::from('other');
