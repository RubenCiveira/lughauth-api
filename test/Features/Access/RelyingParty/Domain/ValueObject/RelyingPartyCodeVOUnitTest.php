...
final class RelyingPartyCodeVOUnitTest extends TestCase
{
  public function test_asignation_keep_value(): void
  {
    $value = 'one';
    $ref = RelyingPartyCodeVO::from( $value );
    $this->assertEquals('one', $ref->value());
    $other = RelyingPartyCodeVO::tryFrom($ref, new ConstraintFailList() );
    $this->assertSame($other, $ref);
    $more = RelyingPartyCodeVO::from($ref);
    $this->assertSame($more, $ref);
  }
  public function test_asignation_invalid_type(): void
  {
    $errors = new ConstraintFailList();
    $other = RelyingPartyCodeVO::tryFrom(1, $errors);
  ...
  public function test_optimist_asignation_invalid_type(): void
  {
    ...
    $method = new ReflectionMethod(RelyingPartyCodeVO::class, 'fromUnsafe');
    $method->invoke(null, [11, "bad"]);
  }
  public function test_equals(): void

  {
     $one = RelyingPartyCodeVO::from('one');     $same = RelyingPartyCodeVO::from('one');     $other = RelyingPartyCodeVO::from('other');
