...
final class RelyingPartyUidVOUnitTest extends TestCase
{
  public function test_asignation_keep_value(): void
  {
    $value = 'one';
    $ref = RelyingPartyUidVO::from( $value );
    $this->assertEquals('one', $ref->value());
    $other = RelyingPartyUidVO::tryFrom($ref, new ConstraintFailList() );
    $this->assertSame($other, $ref);
    $more = RelyingPartyUidVO::from($ref);
    $this->assertSame($more, $ref);
  }
  public function test_asignation_invalid_type(): void
  {
    $errors = new ConstraintFailList();
    $other = RelyingPartyUidVO::tryFrom(1, $errors);
  ...
  public function test_optimist_asignation_invalid_type(): void
  {
    ...
    $method = new ReflectionMethod(RelyingPartyUidVO::class, 'fromUnsafe');
    $method->invoke(null, [11, "bad"]);
  }
  public function test_equals(): void

  {
     $one = RelyingPartyUidVO::from('one');     $same = RelyingPartyUidVO::from('one');     $other = RelyingPartyUidVO::from('other');
