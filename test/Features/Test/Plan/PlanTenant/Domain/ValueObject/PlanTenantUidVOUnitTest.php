...
final class PlanTenantUidVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = PlanTenantUidVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = PlanTenantUidVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = PlanTenantUidVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = PlanTenantUidVO::tryFrom(1, $errors);
