...
final class PlanTenantNameVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = PlanTenantNameVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = PlanTenantNameVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = PlanTenantNameVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = PlanTenantNameVO::tryFrom(1, $errors);
