...
final class SnippetAssetTypeVOUnitTest extends TestCase
{
    public function test_asignation_keep_value(): void
    {
        $value = 'one';
        $ref = SnippetAssetTypeVO::from($value);
        $this->assertEquals('one', $ref->value());
        $other = SnippetAssetTypeVO::tryFrom($ref, new ConstraintFailList());
        $this->assertSame($other, $ref);
        $more = SnippetAssetTypeVO::from($ref);
        $this->assertSame($more, $ref);
    }
    public function test_asignation_invalid_type(): void
    {
        $errors = new ConstraintFailList();
        $other = SnippetAssetTypeVO::tryFrom(1, $errors);
    ...
    public function test_optimist_asignation_invalid_type(): void
    {
        ...
        $method = new ReflectionMethod(SnippetAssetTypeVO::class, 'fromUnsafe');
        $method->invoke(null, [11, "bad"]);
    }
    public function test_equals(): void
    {
        $one = SnippetAssetTypeVO::from('one');
        $same = SnippetAssetTypeVO::from('one');
        $other = SnippetAssetTypeVO::from('other');
