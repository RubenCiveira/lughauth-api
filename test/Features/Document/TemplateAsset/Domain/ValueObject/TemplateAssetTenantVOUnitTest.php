...
final class TemplateAssetTenantVOUnitTest extends TestCase
{
  public function test_asignation_keep_value(): void
  {
    $value = new TenantRef('one');
    $ref = TemplateAssetTenantVO::from( $value );
    $this->assertEquals(new TenantRef('one'), $ref->value());
    $other = TemplateAssetTenantVO::tryFrom($ref, new ConstraintFailList() );
    $this->assertSame($other, $ref);
    $more = TemplateAssetTenantVO::from($ref);
    $this->assertSame($more, $ref);
  }
  public function test_asignation_invalid_type(): void
  {
    $errors = new ConstraintFailList();
    $other = TemplateAssetTenantVO::tryFrom('1', $errors);
  ...
  public function test_optimist_asignation_invalid_type(): void
  {
    ...
    $method = new ReflectionMethod(TemplateAssetTenantVO::class, 'fromUnsafe');
    $method->invoke(null, [11, "bad"]);
  }
  public function test_equals(): void

  {
     $one = TemplateAssetTenantVO::from(new TenantRef('one'));     $same = TemplateAssetTenantVO::from(new TenantRef('one'));     $other = TemplateAssetTenantVO::from(new TenantRef('other'));
