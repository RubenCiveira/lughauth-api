...
final class MetricsRotator
{
    ...
    private function processSeries(string $seriesDir): void
    {
        foreach (array_keys($this->ttlDays) as $partition) {
            ...
            if (!is_dir($partDir)) continue;

            // YYYY
            foreach (glob($partDir.'/*', GLOB_ONLYDIR) ?: [] as $yDir) {
                $y = basename($yDir);
                if (!ctype_digit($y) || strlen($y) !== 4) continue;

                // MM
                foreach (glob($yDir.'/*', GLOB_ONLYDIR) ?: [] as $mDir) {
                    $m = basename($mDir);
                    if (!ctype_digit($m) || (int)$m < 1 || (int)$m > 12) continue;
    ...
    {
        if (!$this->gzipOnRotate) return;
        if (in_array($partition, $this->gzipSkipPartitions, true)) return;
        if (str_ends_with($path, '.gz')) return;
        ...
        if (sprintf('%04d-%02d-%02d', $y, $m, $d) === $today) return;
