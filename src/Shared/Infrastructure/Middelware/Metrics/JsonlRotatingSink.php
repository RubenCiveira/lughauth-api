...
final class JsonlRotatingSink implements MetricsSink
{
    ...
    public function appendBatch(array $batch): void
    {
        ...
        if (!$locked) {
            // Back-pressure 2: el writer está ocupado → adelgaza y luego bloquea normal
            $batch = $this->shedBatch($batch, $this->sheddingStepMs);
    ...
    /** Elimina ficheros > TTL días (jsonl y .gz). */
    ...
    /** Comprime a .gz todos los días distintos del actual que no lo estén ya */
    ...
    /** Downsample oportunista: >2 días → @5m, >14 días → @1h (y borra el original gz si el agregado se crea OK). */
    ...
    private function downsampleGzFile(string $gzFile, int $stepSeconds): void
    {
        ...
        while (!\gzeof($in)) {
            ...
            // Estrategia simple: gauge → última; counter → máximo (monotónico)
            $val = $row['value'];
        ...
        // Renombra tmp → final y borra original
        @\rename($tmp, $out);
        @\unlink($gzFile);
    }
     ...
     * - gauge → última del bucket
     * - counter → máximo del bucket (respeta monotonicidad)
     * - histogram/summary: tal cual (no se colapsa aquí; ajusta si lo necesitas)
     * @param list<array<string,mixed>> $batch
    ...
    private function shedBatch(array $batch, int $stepMs): array
    {
        ...
        foreach ($batch as $row) {
            $typ = (string)($row['type'] ?? 'gauge');
            if ($typ === 'histogram' || $typ === 'summary') { // no colapsamos aquí
                $out[] = $row;
                continue;
            }
            $ts = (int)($row['ts'] ?? 0);
            ...
            if (!isset($map[$key])) {
                ...
                if ($typ === 'counter') {
                    ...
                    $out[$idx] = $row; // nos quedamos con la última
                    $out[$idx]['ts'] = $bucket;
                }
            }
        }
