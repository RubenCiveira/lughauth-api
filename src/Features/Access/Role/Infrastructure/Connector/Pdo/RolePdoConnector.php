...
namespace Civi\Lughauth\Features\Access\Role\Infrastructure\Connector\Pdo;
...
class RolePdoConnector
{
    ...
    public function create(Role $entity, ?Closure $verify = null): Role
    {
        ...
        try {
            try {
                $this->db->execute('INSERT INTO "access_role" ( "uid", "name", "relying_party", "version") VALUES ( :uid, :name, :relyingParty, :version)', [
    ...
    public function update(Role $update): ?Role
    {
        ...
        try {
            try {
                $result = $this->db->execute('UPDATE "access_role" SET "name" = :name , "relying_party" = :relyingParty , "version" = :version WHERE "uid" = :uid and "version" = :_lock_version', [
    ...
    private function checkDuplicates(Role $entity, bool $creation)
    {
        ...
        try {
            ...
            $values = ['relyingParty' => $entity->getRelyingParty()?->uid(), 'name' => $entity->getName(), 'uid' => $entity->uid()];
            if ($this->db->exists('SELECT  "relying_party", "name" from "access_role" where "relying_party" = :relyingParty and "name" = :name and "uid" != :uid', $values)) {
                throw ConstraintException::ofError('not-unique', ['relyingParty', 'name'], [$entity->getRelyingParty()?->uid(), $entity->getName()]);
            }
        } catch (Throwable $ex) {
            $span->recordException($ex);
            throw $ex;
