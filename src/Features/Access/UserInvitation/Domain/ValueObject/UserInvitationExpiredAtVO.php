...
class UserInvitationExpiredAtVO
{
    public static function from(UserInvitationExpiredAtVO|\DateTimeImmutable $value): UserInvitationExpiredAtVO
    {
        return self::fromUnsafe($value);
    }
    public static function tryFrom(mixed $value, ConstraintFailList $list): ?UserInvitationExpiredAtVO
    {
        if ($value instanceof UserInvitationExpiredAtVO) {
            ...
            if ($typed = self::readFromString($value)) {
                ...
                $list->add(new ConstraintFail('wrong_format', ['expiredAt'], [$value], ['\DateTimeImmutable']));
                return null;
            }
        } elseif (is_a($value, \DateTimeImmutable::class)) {
            return new UserInvitationExpiredAtVO($value);
        } else {
            $list->add(new ConstraintFail('wrong_type', ['expiredAt'], [$value], ['\DateTimeImmutable']));
            return null;
        }
    }
    ...
    private static function fromUnsafe(mixed $value): UserInvitationExpiredAtVO
    {
        if ($value instanceof UserInvitationExpiredAtVO) {
            ...
            \assert($candidate instanceof UserInvitationExpiredAtVO);
            return $candidate;
        }
    }
    /**
    ...
    public function equals(?UserInvitationExpiredAtVO $other): bool
    {
