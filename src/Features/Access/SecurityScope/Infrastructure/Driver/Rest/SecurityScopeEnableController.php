...
use Civi\Lughauth\Features\Access\SecurityScope\Infrastructure\Driver\Batch\SecurityScopeTaskEnable;
...
class SecurityScopeEnableController
{
  ...
  #[OA\Patch(path: "/api/access/security-scopes/{uid}/enable", tags: ["Security scope"], description: "Update the value",
    parameters: [
  ...
  {
    $this->logDebug("enable for Security scope");
    $span = $this->startSpan("enable for Security scope");
    $this->sql->begin();
    try {
      if (!isset($args['uid'])) {
  ...
  {
    $this->logDebug("enable all for query Security scope");
    $span = $this->startSpan("enable all for query Security scope");
    try {
      ...
      $res = $this->runner->run(SecurityScopeTaskEnable::class, ['filter' => $filter]);
  ...
  {
    $this->logDebug("Check enable all for query Security scope");
    $span = $this->startSpan("Check enable all for query Security scope");
    try {
      if (!isset($args['uid'])) {
        throw new NotFoundException('-');
      }
  ...
  private function mapSecurityScope(SecurityScopeEnableResult $value): SecurityScopeApiDTO
  {
