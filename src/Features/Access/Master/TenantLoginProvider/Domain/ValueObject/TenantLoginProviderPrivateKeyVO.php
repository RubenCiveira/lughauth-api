...
class TenantLoginProviderPrivateKeyVO
{
    public static function empty(): TenantLoginProviderPrivateKeyVO
    {
        return new TenantLoginProviderPrivateKeyVO(null);
    }
    ...
    public static function tryFrom(mixed $value, ConstraintFailList $list): ?TenantLoginProviderPrivateKeyVO
    {
        if (is_a($value, TenantLoginProviderPrivateKeyVO::class)) {
            ...
            return new TenantLoginProviderPrivateKeyVO($value);
        } elseif (is_string($value)) {
            // If is a ValueObject, we need to append all the errors for the context
            $valid = true;
            foreach (self::rules() as $rule) {
                if ($fail = $rule->check($value)) {
                    $list->add(ConstraintFail::fromRuleFail('privateKey', $fail));
                    $valid = false;
                }
            }
            return $valid ? new TenantLoginProviderPrivateKeyVO($value) : null;
        } else {
            $list->add(new ConstraintFail('wrong_type', ['privateKey'], [$value], ['string']));
            return null;
        }
    }
