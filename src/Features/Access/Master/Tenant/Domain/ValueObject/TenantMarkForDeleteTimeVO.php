...
class TenantMarkForDeleteTimeVO
{
    public static function empty(): TenantMarkForDeleteTimeVO
    {
        return new TenantMarkForDeleteTimeVO(null);
    }
    ...
    public static function tryFrom(mixed $value, ConstraintFailList $list): ?TenantMarkForDeleteTimeVO
    {
        if (is_a($value, TenantMarkForDeleteTimeVO::class)) {
            ...
            return new TenantMarkForDeleteTimeVO($value);
        } elseif (is_string($value)) {
            if ($typed = self::readFromString($value)) {
                return self::tryFrom($typed, $list);
            } else {
                $list->add(new ConstraintFail('wrong_format', ['markForDeleteTime'], [$value], ['\DateTimeImmutable']));
                return null;
            }
        } elseif (is_a($value, \DateTimeImmutable::class)) {
            return new TenantMarkForDeleteTimeVO($value);
        } else {
            $list->add(new ConstraintFail('wrong_type', ['markForDeleteTime'], [$value], ['\DateTimeImmutable']));
            return null;
        }
    }
    ...
    private function __construct(
        private readonly ?\DateTimeImmutable $markForDeleteTime
    ) {
    }
