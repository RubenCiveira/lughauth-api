...
use Civi\Lughauth\Features\Access\Master\User\Infrastructure\Driver\Batch\UserTaskUnlock;
...
class UserUnlockController
{
    ...
    #[OA\Patch(
        path: "/api/access/users/{uid}/unlock",
        tags: ["User"],
    ...
    {
        $this->logDebug("unlock for User");
        $span = $this->startSpan("unlock for User");
        $this->sql->begin();
        try {
            if (!isset($args['uid'])) {
            ...
            $uid = $args['uid'];
            $result = $this->unlockUsecase->unlock($uid);
            $this->sql->commit();
    ...
    public function unlockAllForQuery(ServerRequestInterface $request, ResponseInterface $response, array $args): ResponseInterface
    {
        $this->logDebug("unlock all for query User");
        $span = $this->startSpan("unlock all for query User");
        try {
            ...
            $res = $this->runner->run(UserTaskUnlock::class, ['filter' => $filter]);
    ...
    public function checkUnlockAllForQueryStatus(ServerRequestInterface $request, ResponseInterface $response, array $args): ResponseInterface
    {
        $this->logDebug("Check unlock all for query User");
        $span = $this->startSpan("Check unlock all for query User");
        try {
            if (!isset($args['uid'])) {
                throw new NotFoundException('-');
            }
    ...
    private function mapUser(UserUnlockResult $value): UserApiDTO
    {
