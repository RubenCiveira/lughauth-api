...
class TenantLoginProviderEnableUsecase
{
    ...
    public function allowEnable(?TenantLoginProviderRef $ref = null): Allow
    {
        ...
        try {
            $result = $this->dispatcher->dispatch(new TenantLoginProviderEnableAllowDecision(Allow::allowed('enable', 'Allowed to Tenant login provider by default')));
    ...
    public function enable(string $uid): TenantLoginProviderEnableResult
    {
        ...
        try {
            ...
            if (!$allow->allowed) {
                throw new UnauthorizedException($allow->reason ?? 'Not allowed to Enable Tenant login provider');
            }
            if (!$original = $this->visibility->retrieveVisibleForUpdate($ref)) {
                throw new NotFoundException($uid);
            }
            $this->dispatcher->dispatch(new TenantLoginProviderEnableCheck($original));
