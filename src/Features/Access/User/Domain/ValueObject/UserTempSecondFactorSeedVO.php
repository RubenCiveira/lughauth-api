...
class UserTempSecondFactorSeedVO
{
    public static function fromCypheredText(AesCypherService $cypher, string $value): UserTempSecondFactorSeedVO
    {
        return self::from('cyphered://' . $value);
    }
    public static function fromPlainText(AesCypherService $cypher, string $value): UserTempSecondFactorSeedVO
    {
        return self::from(self::isCyphered($value) ? $value : 'cyphered://' . $cypher->encryptForAll($value));
    }
    public static function tryFromCypheredText(AesCypherService $cypher, string $value, ConstraintFailList $list): UserTempSecondFactorSeedVO
    {
        return self::tryFrom('cyphered://' . $value, $list);
    }
    public static function tryFromPlainText(AesCypherService $cypher, string $value, ConstraintFailList $list): UserTempSecondFactorSeedVO
    {
    ...
    public static function empty(): UserTempSecondFactorSeedVO
    {
        return new UserTempSecondFactorSeedVO(null);
    }
    public static function from(UserTempSecondFactorSeedVO|string|null $value): UserTempSecondFactorSeedVO
    {
        if (is_a($value, UserTempSecondFactorSeedVO::class)) {
    ...
    public static function tryFrom(mixed $value, ConstraintFailList $list): ?UserTempSecondFactorSeedVO
    {
        if (is_a($value, UserTempSecondFactorSeedVO::class)) {
            // If is a ValueObject, its already validated... nothing to append
            return $value;
        } elseif (!$value) {
            return new UserTempSecondFactorSeedVO($value);
            ...
            return $valid ? new UserTempSecondFactorSeedVO($value) : null;
        } else {
