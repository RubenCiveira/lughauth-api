...
{
    public static function empty(): UserLanguageVO
    {
        return new UserLanguageVO(null);
    }
    public static function from(UserLanguageVO|string|null $value): UserLanguageVO
    {
        if (is_a($value, UserLanguageVO::class)) {
    ...
    public static function tryFrom(mixed $value, ConstraintFailList $list): ?UserLanguageVO
    {
        if (is_a($value, UserLanguageVO::class)) {
            // If is a ValueObject, its already validated... nothing to append
            return $value;
        } elseif (!$value) {
            return new UserLanguageVO($value);
        } elseif (is_string($value)) {
            // If is a ValueObject, we need to append all the errors for the context
            $valid = true;
            foreach (self::rules() as $rule) {
                if ($fail = $rule->check($value)) {
                    $list->add(ConstraintFail::fromRuleFail('language', $fail));
                    $valid = false;
                }
            }
            return $valid ? new UserLanguageVO($value) : null;
        } else {
            $list->add(new ConstraintFail('wrong_type', ['language'], [$value], ['string']));
            return null;
        }
    }
