...
/**
 * @template-extends ArrayIterator<int, Theme>
 */
class ThemeSlide extends \ArrayIterator
{
    private readonly ?Theme $last;
    private readonly ThemeCursor $nextCursor;
    public function __construct(
        ...
        $cursor = $this->cursor;
        $last = $this->last;
        $this->nextCursor = new ThemeCursor(
    ...
    public function nextCursor(): ThemeCursor
    {
    ...
    public function map(\Closure $fn): ThemeSlide
    {
        return new ThemeSlide($this->next, $this->cursor, array_map($fn, $this->values));
    }
    public function slide(\Closure $fn): ThemeSlide
    {
        ...
        if (null !== $cursor && count($collected) == $cursor->limit()) {
            ...
            return new ThemeSlide($next, $cursor, $collected);
        } else {
            return $this;
        }
    }
