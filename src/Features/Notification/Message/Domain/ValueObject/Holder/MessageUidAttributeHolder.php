...
trait MessageUidAttributeHolder
{
    ...
    public function getUidOrCurrent(MessageUidVO $uid): MessageUidVO
    {
        if ($this->uidAssigned) {
            ...
            return MessageUidVO::from($this->uid);
        } else {
            return $uid;
        }
    }
    ...
    {
        return $this->uid instanceof MessageUidVO ? $this->uid->value() : $this->uid;
    }
    ...
    protected function withAssertedUidRules(self $value, ConstraintFailList $errorsList): void
    {
        if ($value->uidAssigned = $this->uidAssigned) {
            $value->uid = MessageUidVO::tryFrom($this->uid, $errorsList);
        }
    }
}
